...
* `command-buffer-record`: adds a queue recording API on top of
  ...
  directory is a standalone CMake project, holding its own documentation, tests
  and CI workflow, so that it can be moved to a repository of its own. It is
  not part of the collection build below.
